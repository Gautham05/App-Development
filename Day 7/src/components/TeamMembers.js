...
const TeamMembers = ({ members, dispatch }) => {
  ...
  useEffect(() => {
    ...
    if (storedMembers) {
      ...
      setNewMember({ name: '', role: '' }); // Initialize newMember with empty values
    }
  }, [dispatch]);
      ...
      {isSidebarOpen && (
        <div className="sidebar sidebar-right">
          ...
          <ul>
            ...
            <li><Link to="/teammembers">Members</Link></li>
          </ul>
        </div>
      )}
      <div>
      ...
      <ul className='member-ul'>
        {combinedMembers.map((member) => (
          <li className='member-li' key={member.id}>
            ...
            <svg onClick={() => handleEdit(member.id)} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#003975" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-pencil"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>
            <svg onClick={() => handleDeleteMember(member.id)} xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#003975" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-trash-2"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/><line x1="10" x2="10" y1="11" y2="17"/><line x1="14" x2="14" y1="11" y2="17"/></svg>
            </div>
          </li>
        ))}
      </ul>
